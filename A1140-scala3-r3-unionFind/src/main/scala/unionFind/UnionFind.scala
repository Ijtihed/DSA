...
object solver:
  ...
  def solve(nofVertices: Int, edges: collection.Seq[(Int, Int, Int)]): Option[Int] =
    ...
    for v <- 0 until nofVertices do sets.makeSet(v)
...
class UnionFind[E]:
  private val parent = collection.mutable.Map[E, E]()
  private val rank = collection.mutable.Map[E, Int]()
  private var _nofSets = 0
  ...
  def makeSet(element: E): Boolean =
    if !parent.contains(element) then
  ...
  def findSet(element: E): E =
    if parent(element) != element then parent(element) = findSet(parent(element))
    parent(element)
  end findSet
  ...
  def union(element1: E, element2: E): Unit =
    val root1 = findSet(element1)
    val root2 = findSet(element2)
    if root1 != root2 then
      if rank(root1) > rank(root2) then parent(root2) = root1
      else if rank(root1) < rank(root2) then parent(root1) = root2
      else
        parent(root2) = root1
        rank(root1) += 1
      _nofSets -= 1
